            ...
            <div key={reservation.reservationId} className="bg-white rounded-xl shadow p-6">
              <div className="flex justify-between items-start mb-3">
                <h3 className="font-bold text-gray-800 text-lg">
                  {reservation.game.homeTeam} vs {reservation.game.awayTeam}
                </h3>
              ...
              <div className="text-sm text-gray-500 flex flex-col gap-1 mb-4">
                <p>예매 번호: {reservation.reservationNumber}</p>
                <p>경기 시간: {new Date(reservation.game.startTime).toLocaleString('ko-KR')}</p>
                <p>결제 금액: {reservation.totalAmount.toLocaleString()}원</p>
                <p>결제 시간: {new Date(reservation.paidAt).toLocaleString('ko-KR')}</p>
              </div>
